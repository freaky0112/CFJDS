...
namespace CFJDS {
    class BiultCFGZS {
        ...
        private void addTitle(BiultReportForm brf,DataCFSJ data) {
            ...
            pFontSize = 12;
            pFontBold = 0;//设置细体
            pText = "青土资告字〔2014〕第"+data.Code + String.Format("{0:0000}", data.No)+"号";
            addLine(brf);
        }
        /// <summary>
        /// 输入正文
        ...
        private void addText(BiultReportForm brf, DataCFSJ data) {
            ...
            pText += data.Location;
            pText += "非法占用土地";
            pText += data.Area.ToString();
            pText += "平方米，用于建房。经核对青田县";
            pText += data.Town;
            ...
            if (data.IsnotConfiscate) {
                pText = "1.  没收你非法占用 " + data.ConfiscateArea.ToString() + "平方米的建筑物。";
                addLine(brf);
                pText = "    2.  ";
                addTxt(brf);
            }
